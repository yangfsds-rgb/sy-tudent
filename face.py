...
class FocusAnalyzer:
    def __init__(self, yolo_weights="weights/yolov9s.pt"):
        # 硬件配置
        ...
        # 行为权重设置，三种行为的权重和为1
        self.behavior_weights = {'rise_hand': 0.1, 'read': 0.6, 'write': 0.3}
    ...
    def _init_yolo(self, weights_path):
        """直接加载 YOLOv9 模型"""
        model = DetectMultiBackend(weights_path, device=self.device)  # 加载权重
        return model
    ...
    def process_frame(self, frame, result_txt="results.txt", time_step=1):
        """同步处理管道，包括时间特征"""
        # 并行执行检测
        detected_boxes = self._detect_behavior(frame.copy())

        # 初始化关注度
        focus_scores = {behavior: 0 for behavior in self.class_names}

        # 存储历史关注度
        current_focus_scores = {behavior: [] for behavior in self.class_names}

        # 打开文件进行写入
        with open(result_txt, "a") as f:
            f.write(f"Processing frame: \n")
            ...
            for box in detected_boxes:
                ...
                if current_embedding is not None:
                    ...
                    # 将当前帧的专注度加入时间序列
                    current_focus_scores[label].append(focus_score)

                    # 计算时间加权平均，较新的帧更高权重
                    time_weighted_focus_score = np.mean(current_focus_scores[label][-10:])  # 使用过去10帧进行加权

                    focus_scores[label] = time_weighted_focus_score  # 更新时间序列计算后的专注度

                    # 打印行为类别，置信度，情绪类别，情绪置信度和专注度
                    print(f"Person ID: {person_id}, Behavior: {label}, Confidence: {confidence:.2f}, Emotion: {emotion}, "
                          f"Emotion Confidence: {emotion_confidence:.2f}, Focus Score: {time_weighted_focus_score:.2f}")
                    ...
                    # 标注行为类别和专注度得分
                    cv2.putText(frame, f"{label}: {time_weighted_focus_score:.2f}", (int(x1), int(y1) + 30),  # 行为类别 + 专注度
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    ...
    def analyze_video(self, video_path, output_txt="results.txt", output_video=None):
        """分析视频并保存结果到文本文件和视频文件"""
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            print(f"Error: Unable to load video at {video_path}")
            return
        ...
        while True:
            ...
            # 如果指定了输出视频路径，则保存标注后的图像到视频文件
            if output_video:
                if out is None:
                    out = cv2.VideoWriter(output_video, fourcc, 30, (frame.shape[1], frame.shape[0]))
...
if __name__ == "__main__":
    ...
    # 分析视频并保存结果
    analyzer.analyze_video("QQ2025223-175928.mp4", output_txt="results.txt", output_video="output_video.avi")
