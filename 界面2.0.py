...
class FocusAnalysisApp:
    ...
    def show_behavior_emotion_plot(self):
        try:
            ...
            if df is None or df.empty:
                messagebox.showwarning("警告", "没有可用的分析数据")
                return
            ...
            ax1.set_title('学生行为-情绪时序变化', pad=10, fontsize=12)
        ...
        def analyze_frames():
            ...
            if self.video_source == "image":
                try:
                    ...
                    self.append_result(f"图片分析错误: {str(e)}\n")
                finally:
                    self.stop_analysis()
    ...
    def save_results(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=default_name  # 添加默认文件名
        )
    ...
    def open_image(self):
        ...
        if file_path:
            self.stop_analysis()
            self.video_source = "image"
            self.current_image_path = file_path  # 新增路径记录
