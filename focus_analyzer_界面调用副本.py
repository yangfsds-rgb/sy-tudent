    ...
    def process_frame(self, frame, result_txt="results.txt"):
        ...
        with open(result_txt, "a") as f:
            ...
            for box in detected_boxes:
                ...
                focus_scores[label] = focus_score  # 将关注度直接赋值为每个行为的独立值

                # 打印行为类别，置信度，情绪类别，情绪置信度和专注度
                print(f"Behavior: {label}, Confidence: {confidence:.2f}, Emotion: {emotion}, "
                      f"Emotion Confidence: {emotion_confidence:.2f}, Focus Score: {focus_score:.2f}")

                # 将结果写入文件
                f.write(f"Behavior: {label}, Confidence: {confidence:.2f}, Emotion: {emotion}, "
                        f"Emotion Confidence: {emotion_confidence:.2f}, Focus Score: {focus_score:.2f}\n")

                # 在图像中标注行为和关注度
                cv2.putText(frame, f"{label}: {focus_score:.2f}", (int(x1), int(y1) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)  # 绘制矩形框

            # 打印每个行为的关注度
            for behavior, focus_score in focus_scores.items():
                print(f"{behavior} Focus: {focus_score:.2f}")

        # 返回处理后的图像和每个行为的关注度
        return frame, focus_scores

    def analyze_image(self, image_path, output_txt="results_images.txt", output_image=None):
        """分析单张图像并保存结果到文本文件、CSV 文件和图像文件"""
        ...
        # 在图像上标注专注度分数
        for behavior, focus_score in focus_scores.items():
            cv2.putText(annotated_frame, f"{behavior}: {focus_score:.2f}%", (20, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        # 如果指定了输出图像路径，则保存标注后的图像
        if output_image:
...
if __name__ == "__main__":
    ...
    # 分析图像并保存结果
    analyzer.analyze_image("./spark.png")
